...
public class AddressService : IAddressService
{
    ...
    public SearchAddressModel[] Search(Int64 parentObjectId, string? query)
    {
        ...
        if (query == null)
        {
            addresses = (from AsAdmHierarchy in _context.AsAdmHierarchies
                ...
                from AsHouse in _context.AsHouses.Where(house =>
                        AsAdmHierarchy.Objectid == house.Objectid && house.Isactual == 1 && house.Isactive == 1)
                    .DefaultIfEmpty()
                from AsAddrObj in _context.AsAddrObjs.Where(obj =>
                    AsAdmHierarchy.Objectid == obj.Objectid && obj.Isactual == 1 && obj.Isactive == 1).DefaultIfEmpty()
    ...
    {
        var address =
            _context.AsAddrObjs.FirstOrDefault(
                ad => ad.Objectguid == objectGuid && ad.Isactual == 1 && ad.Isactive == 1);
        var houseAddress = _context.AsHouses.FirstOrDefault(house =>
            house.Objectguid == objectGuid && house.Isactual == 1 && house.Isactive == 1);
        ...
        {
            houseFromHouses = _context.AsHouses.FirstOrDefault(house =>
                house.Objectid == houseAddress.Objectid && house.Isactual == 1 && house.Isactive == 1);
        }
        ...
        {
            var curAddrObject = _context.AsAddrObjs.FirstOrDefault(obj =>
                obj.Objectid == el.Objectid && obj.Isactual == 1 && obj.Isactive == 1);
            var curHouseObject = _context.AsHouses.FirstOrDefault(house =>
                house.Objectid == el.Objectid && house.Isactual == 1 && house.Isactive == 1);
    ...
    {
        var currentObject =
            _context.AsAdmHierarchies.FirstOrDefault(obj => obj.Objectid == objectId && obj.Isactive == 1);
        ...
        var parentObject =
            _context.AsAdmHierarchies.FirstOrDefault(obj =>
                obj.Objectid == currentObject.Parentobjid && obj.Isactive == 1);
